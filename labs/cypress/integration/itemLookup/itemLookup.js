...
import ItemLookupPage from '../../support/pages/ItemLookupPage';
...
And('The user clicks on the Item Lookup menu', () => {
  ItemLookupPage.itemLookupButton.click();
});

And('Validate the Item Search Page', () => {
  ItemLookupPage.validateItemLookupPage();
});

Then('Click the Valid Item name in touchpad keyboard', (datatable) => {
  ItemLookupPage.itemLookupSearchStub();
  datatable.hashes().forEach((element) => {
    ItemEntry.enterItemAndClickSearch(element.keyword);
  });
});

Then('Select any one category from the category list', () => {
  ItemLookupPage.selectCatagory();
});

Then('Click on any item from item search list', () => {
  ItemLookupPage.addItemToCart();
});
...
Then('Validate item not found result on item lookup', () => {
  ItemLookupPage.validateItemNotFound();
});

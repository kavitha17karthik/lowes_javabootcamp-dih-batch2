...
import { CARD_LAR, CARD_TENDER } from '../../support/constants';
...
And('Authorization Loader get enabled LAR', () => {
  TenderPage.tenderPageStubsBFF(CARD_LAR);
  TenderPage.tenderCapture(CARD_LAR);
});

And('Authorization Loader get enabled failed to approve', () => {
  TenderPage.failedActivation(controller, CARD_LAR);
  TenderPage.tenderCapture(CARD_LAR);
});
...
And('Validate tender complete', () => {
  TenderPage.tenderPaidViaCard(CARD_LAR);
});
